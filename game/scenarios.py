...
class Scenario:
    ...
    bonus: str | None = None
    # The rules this day was GENERATED under, recorded rather than inferred.
    #
    # A stored day has to replay exactly as it was scored -- that is migration 032's whole
    # reason for storing a scenario at all -- and both of these change what a state string
    # decodes to or what the match does with it. Reading them off the kind instead would
    # have been wrong for the one real case that already exists: a day generated by
    # yesterday's deploy carries a CURRENT kind under the OLD rules, so the kind cannot
    # tell them apart. False on every day generated before each rule landed.
    deal_unique: bool = False      # a franchise-season is dealt at most once per draft
    impact_plays: bool = False     # the Impact Player is actually substituted in
    ...
    **The BONUS is chosen first and the KIND accommodates it**, which is the one place these
    ...
    if kind == WIN_BY_RUNS:
        return Scenario(kind, opposition_fs_id, opposition_name, stage,
                        ...
                        deal_unique=True, impact_plays=True)
