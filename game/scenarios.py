...
#
# A stored day has to replay exactly as it was scored -- migration 032's whole reason for
# storing a scenario rather than deriving it -- and every entry below changes what a state
# string decodes to, or what the match does with it. So the version is RECORDED on the day
# and never inferred from anything else. Inferring was tried and is wrong for a case that
# genuinely happened: a day generated by the previous deploy carries a CURRENT kind under
# the OLD rules, so the kind cannot tell them apart.
#
#   0  the original. Franchise-seasons dealt WITH replacement, so one squad could come up
#      five times in twelve picks; the drafted Impact Player was a scorecard tag and never
#      took the field; the match was seeded from the DATE AND ACCOUNT ID.
#   1  a franchise-season is dealt at most once per draft, and the Impact Player really is
#      substituted in at the innings break.
#   2  the match is seeded from the DRAFTED TWELVE instead of the account. Measured cause:
#      on one day, one identical twelve played on forty different account seeds returned
#      margins from -5 to +96 and met the objective 17 times in 40 -- so who you signed up
#      as moved the result more than which players you picked, and no amount of re-drafting
#      could escape a bad one. Seeding from the team makes the result a function of the
#      choices instead, and two players who draft the same twelve now get the same match.
#
# Adding a version means adding a line here, a property below, and nothing else.

RULES_LEGACY = 0
RULES_FULL_MATCH = 1
RULES_TEAM_SEEDED = 2

# What a day generated right now records.
RULES = RULES_TEAM_SEEDED
...
class Scenario:
    ...
    bonus: str | None = None
    # WHICH RULES this day was generated under. See RULES below for what each version
    # means. Zero -- the default -- is the oldest, which is what an absent value means on
    # every day stored before this was recorded.
    rules: int = RULES_LEGACY
    ...
    # The individual rules, derived from the version rather than stored beside it. Call
    # sites ask the question they actually care about ("does the Impact Player play?") and
    # never compare an integer, so adding a version is one line here and none anywhere
    # else -- which is the whole reason this replaced a growing row of booleans.

    @property
    def deal_unique(self) -> bool:
        """A franchise-season is dealt at most once per draft."""
        return self.rules >= RULES_FULL_MATCH

    @property
    def impact_plays(self) -> bool:
        """The Impact Player is really substituted in, rather than being a scorecard tag."""
        return self.rules >= RULES_FULL_MATCH

    @property
    def team_seeded_match(self) -> bool:
        """The match dice come from the drafted twelve rather than from the account id."""
        return self.rules >= RULES_TEAM_SEEDED
