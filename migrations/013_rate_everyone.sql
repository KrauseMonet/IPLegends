-- SPEC 7.10. Everyone who played gets a rating, and the shrinkage learns to count matches.
--
-- A VIEW REPLACEMENT AND NOTHING ELSE, as 010-012 were.
...
blended as (
    select
        ...
        greatest(
            ...
            -- [A67] Reputation as a floor: it may lift, and may never drag.
            0.85 * c.career_merit
        ) as blended_merit
    from career c
),
...
comment on view player_season_rating is
    'SPEC 7.8-7.10 rating. EVERY player-season that faced or bowled a ball is rated (A65 - the A33 gate no longer removes a card, only records that the estimate is thin). Runs above par per match (A54), disciplines added (A55), Player of the Match priced in (A56), a continuous all-rounder term (A59), shrunk on MATCHES for the per-match quantity (A66), with the career acting as a FLOOR rather than a blend (A67 - it may lift a season and may never drag one). Integer 70-99, percentile-anchored (A58/A60). k = 100 balls, K_MATCHES = 6, POTM_RUNS = 12, ALLROUNDER_RUNS = 5, CAREER_FLOOR = 0.85 and the anchors live here and nowhere else.';
